...
list_of_dataframes <- vector("list", length(unique(df$Tree))*length(unique(df$RF))) #we again assume that all trees are always sampled with the same amount of radial files
i <- 1 #a counter

for(tree in unique(df$Tree)) {
  for(rf in unique(df$RF)) {
    
    flux_tree_rf <- filter(df, Tree == tree, RF ==rf) # subsets big data frame by cell count data tree and radial file
