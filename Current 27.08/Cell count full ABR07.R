...
list_of_dataframes <- vector("list", length(unique(df$annee))*length(unique(df$jour)))
i <- 1 #a counter

for(year in unique(df$annee)){ #works with data from different years
  
  start_date <- paste(c(as.character(year-1), "-12-31"), collapse= "")  #last day of the year before
  
  for(day in unique(df$jour)){
      
      #filter by year and day
      temp <- filter(df, annee== year, jour == day)
...
save(df, file = "cell_count_full_ABR07.RData")
