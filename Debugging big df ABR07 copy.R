...
######We add rows for missing samples in cellular data
#Tree 50 is missing samples 10 & 11 of year 2007, we add the relevant rows in the dataframe with NAs where appropriate
for(i in 1:3){
  for(j in 10:11) {
...
#Tree 44 is missing sample 24 of year 2007, we add the relevant rows in the dataframe with NAs where appropriate
for(i in 1:3){
...
################
#We replace all the missing cell values in ABR_07
...
#################Insert cell count data and meteorological data of ABR_07 into same data frame which will be called df############################
...
Sample_i_plus_1 <- filter(df, Sample == 1)
